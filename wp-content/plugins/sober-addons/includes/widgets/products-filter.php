...
class Sober_Products_Filter_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			esc_html__( 'Sober - Products Filter', 'sober' ),
			array(
				'classname'                   => 'sober-products-filter-widget woocommerce',
				'description'                 => esc_html__( 'WooCommerce products filter.', 'sober' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['filter'] ) ) {
			...
			if ( ! $instance['ajax'] || ! $instance['instant'] ) {
				echo '<button type="submit" value="' . esc_attr__( 'Filter', 'sober' ) . '" class="button filter-button">' . esc_html__( 'Filter', 'sober' ) . '</button>';
			}

			if ( $instance['reset_button'] ) {
				echo '<button type="reset" value="' . esc_attr__( 'Reset Filter', 'sober' ) . '" class="button alt reset-button">' . esc_html__( 'Reset Filter', 'sober' ) . '</button>';
			}
		...
		foreach ( $active_filters as $filter ) {
			...
			foreach ( $terms as $term ) {
				switch ( $filter['source'] ) {
					...
					case 'rating':
						$text = _n( 'Rated %d star', 'Rated %d stars', $term, 'sober' );
				...
				if ( ! empty( $text ) ) {
					$list[] = sprintf(
						'<a href="#" class="remove-filtered" data-name="%s" data-value="%s">
	...
	public function display_filter( $filter ) {
		...
		if ( 'attribute' == $filter['source'] ) {
			...
			$args['all']        = sprintf( esc_html__( 'Any %s', 'sober' ), wc_attribute_label( $attr->attribute_label ) );
			...
			$args['all'] = sprintf( esc_html__( 'Select a %s', 'sober' ), $taxonomy->labels->singular_name );
		} else {
			$args['all'] = esc_html__( 'All Products', 'sober' );
		}
	...
	protected function get_filter_options( $filter ) {
		...
		switch ( $filter['source'] ) {
			...
			case 'product_group':
				$filter_groups = array(
					'best_sellers' => esc_attr__( 'Best Sellers', 'sober' ),
					'new'          => esc_attr__( 'New Products', 'sober' ),
					'sale'         => esc_attr__( 'Sale Products', 'sober' ),
					'featured'     => esc_attr__( 'Hot Products', 'sober' ),
				);

				if ( 'dropdown' != $filter['display'] ) {
					$options[''] = array(
						'name'  => esc_attr__( 'All Products', 'sober' ),
						'count' => 0,
						'id'    => 0,
						'level' => 0,
					);
	...
	protected function filter_search_box( $filter ) {
		if ( 'attribute' == $filter['source'] ) {
			...
			$placeholder = __( 'Search', 'sober' ) . ' ' . strtolower( $attributes[ $filter['attribute'] ] );
		} else {
			$sources     = $this->get_filter_source_options();
			$placeholder = __( 'Search', 'sober' ) . ' ' . strtolower( $sources[ $filter['source'] ] );
		}
	...
	public function form( $instance ) {
		...
		$this->setting_field( array(
			'type'  => 'text',
			'name'  => 'title',
			'label' => esc_html__( 'Title', 'sober' ),
			'value' => $instance['title'],
		) );
		?>

		<div class="sober-products-filter-form__sub-nav">
			<button type="button" data-section="filters" class="button-link active"><?php esc_html_e( 'Filters', 'sober' ); ?></button> |
			<button type="button" data-section="options" class="button-link"><?php esc_html_e( 'Options', 'sober' ); ?></button>
		</div>

		<p><hr/></p>

		<div class="sober-products-filter-form__section active" data-section="filters">
			<p class="sober-products-filter-form__message <?php echo ! empty( $instance['filter'] ) ? 'hidden' : '' ?>"><?php esc_html_e( 'There is no filter yet.', 'sober' ) ?></p>
			...
			<p class="sober-products-filter-form__section-actions">
				<button type="button" class="sober-products-filter-form__add-new button-link" data-name="<?php echo esc_attr( $this->get_field_name( 'filter' ) ); ?>" data-count="<?php echo count( $instance['filter'] ) ?>">+ <?php esc_html_e( 'Add a new filter', 'sober' ) ?></button>
			</p>
		</div>
		...
		<div class="sober-products-filter-form__section" data-section="options">
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'ajax',
				'label' => esc_html__( 'Use ajax for filtering', 'sober' ),
				'value' => $instance['ajax'],
			) );
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'instant',
				'label' => esc_html__( 'Filtering products instantly', 'sober' ),
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'change_url',
				'label' => esc_html__( 'Update URL', 'sober' ),
			...
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'reset_button',
				'label' => esc_html__( 'Display the Reset button', 'sober' ),
			...
			$this->setting_field( array(
				...
				'label' => esc_html__( 'Show active filters on top', 'sober' ),
				...
				<div class="sober-products-filter-form__filter-top">
					<div class="sober-products-filter-form__filter-actions">
						<button type="button" class="sober-products-filter-form__remove-filter button-link button-link-delete">
							<span class="screen-reader-text"><?php esc_html_e( 'Remove filter', 'sober' ) ?></span>
	...
	protected function get_filter_source_options() {
		$sources = array(
			'product_group' => esc_html__( 'Group', 'sober' ),
			'price'         => esc_html__( 'Price', 'sober' ),
			'attribute'     => esc_html__( 'Attributes', 'sober' ),
			'rating'        => esc_html__( 'Rating', 'sober' ),
		);
	...
	protected function get_filter_display_options( $source = 'product_cat' ) {
		$options = array(
			'price' => array(
				'slider' => esc_html__( 'Slider', 'sober' ),
				'ranges' => esc_html__( 'Ranges' ),
			),
			'attribute' => array(
				'auto'       => esc_html__( 'Auto', 'sober' ),
				'dropdown'   => esc_html__( 'Dropdown', 'sober' ),
				'list'       => esc_html__( 'Vertical List', 'sober' ),
				'h-list'     => esc_html__( 'Horizontal List', 'sober' ),
				'checkboxes' => esc_html__( 'Checkbox List', 'sober' ),
			),
			'rating' => array(
				'dropdown'   => esc_html__( 'Dropdown', 'sober' ),
				'checkboxes' => esc_html__( 'Checkbox List', 'sober' ),
	...
	protected function get_filter_fields_settings() {
		...
		$this->filter_settings = array(
			'name' => array(
				'type' => 'text',
				'label' => __( 'Filter Name', 'sober' ),
			),
			'source' => array(
				'type' => 'select',
				'label' => __( 'Filter By', 'sober' ),
				'options' => $this->get_filter_source_options(),
			),
			'attribute' => array(
				'type' => 'select',
				'name' => 'attribute',
				'label' => __( 'Attribute', 'sober' ),
			...
			'display' => array(
				'type' => 'select',
				'label' => __( 'Display Type', 'sober' ),
				'options' => $this->get_filter_display_options(),
			),
			'ranges' => array(
				'type' => 'textarea',
				'label' => __( 'Ranges', 'sober' ),
				'desc' => __( 'Each range on a line, separate by the <code>-</code> symbol. Do not include the currency symbol. You can also use <code>></code> and <code><</code> at the begining of lines.', 'sober' ),
				'condition' => array(
					'display' => 'ranges',
					'source'  => 'price',
				),
			...
			'multiple' => array(
				'type' => 'select',
				'label' => __( 'Selection Type', 'sober' ),
				'options' => array(
					0 => __( 'Single select', 'sober' ),
					1 => __( 'Multiple select', 'sober' ),
				),
				'condition' => array(
					'source!'  => ['product_group', 'price'],
					'display!' => ['dropdown', 'slider', 'ranges'],
				),
			),
			'query_type' => array(
				'type' => 'select',
				'label' => __( 'Query Type', 'sober' ),
				'options' => array(
					'and' => __( 'AND', 'sober' ),
					'or' => __( 'OR', 'sober' ),
				),
			...
			'show_counts' => array(
				'type' => 'checkbox',
				'label' => __( 'Show product counts', 'sober' ),
				'condition' => array(
					'source!' => array( 'price', 'product_group' ),
				),
			),
			'searchable' => array(
				'type' => 'checkbox',
				'label' => __( 'Show the search box', 'sober' ),
				'condition' => array(
			...
			'scrollable' => array(
				'type' => 'checkbox',
				'label' => __( 'Limit the height of items list (scrollable)', 'sober' ),
				'condition' => array(
					'display' => array( 'list', 'checkboxes' ),
				),
			),
