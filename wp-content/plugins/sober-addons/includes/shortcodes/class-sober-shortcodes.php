		...
		if ( 'category' == $atts['filter'] ) {
			...
			if ( $categories && ! is_wp_error( $categories ) ) {
				...
				if ( $atts['show_all'] ) {
					...
					$atts['category'] = $atts['category'][0];
				}

				// First tab
				if ( 'isotope' == $atts['filter_type'] ) {
					array_unshift( $filter, '<li data-filter="*" class="line-hover active">' . esc_html__( 'All Products', 'sober' ) . '</li>' );
					// $filter[] = '<li data-filter="*" class="line-hover active">' . esc_html__( 'All Products', 'sober' ) . '</li>';
				} elseif ( $atts['show_all'] ) {
					array_unshift( $filter, '<li data-filter="' . esc_attr( $atts['category'] ) . '" class="line-hover active">' . esc_html__( 'All Products', 'sober' ) . '</li>' );
					// $filter[] = '<li data-filter="" class="line-hover active">' . esc_html__( 'All Products', 'sober' ) . '</li>';
				}
			}
		} else {
